-- Atividades secundárias ligadas por post_activities também devem sair do
-- feed independente; caso contrário Cardio + Musculação + Sauna apareceriam
-- duplicados abaixo do post combinado.
...
as $$
  ...
  where a.publication_state = 'shared'
    ...
    and not exists (
      select 1 from public.post_activities promoted_link
       where promoted_link.activity_id = a.id
    )
