-- =====================================================================
-- Hardening pós-advisors:
--   * envolve auth.uid() em (select auth.uid()) (init plan otimizado)
--   * adiciona índices em FKs sem cobertura
--   * remove SELECT policy nos buckets públicos (URLs públicas
--     continuam funcionando; só não dá pra LISTAR conteúdo)
-- =====================================================================
...
-- 2. Políticas com (select auth.uid()) — recriar
-- profiles
...
-- 3. Storage: remover SELECT (lista) — URL pública continua funcionando.
-- Para upload/update/delete, manter mas otimizar com (select auth.uid()).
