create or replace function public.get_founder_status(p_user_ids uuid[] default null)
returns table (
  user_id uuid,
...
as $$
  ...
  select
    ranked.user_id,
    ranked.founder_rank,
    ranked.founder_rank <= 100 as is_founder
  from ranked
  where (p_user_ids is null or ranked.user_id = any(p_user_ids))
    and ranked.founder_rank <= 100;
$$;

comment on function public.get_founder_status(uuid[]) is
  'Returns Founder relic eligibility for the first 100 active Gym Circle profiles by signup order.';
...
revoke all on function public.get_founder_status(uuid[]) from public, anon;
revoke all on function public.get_achievement_rarity_summary() from public, anon;
grant execute on function public.get_founder_status(uuid[]) to authenticated;
